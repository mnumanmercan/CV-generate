...
const prismaMock = {
  user: {
    ...
    create: vi.fn(async ({ data }: { data: Record<string, unknown> }) => {
      const user = {
        ...
        emailVerified: false,
        ...data,
      } as typeof db.users extends Map<string, infer V> ? V : never
  ...
  token: {
    findUnique: vi.fn(
      async ({ where, include }: { where: { tokenHash: string }; include?: { user: true } }) => {
    ...
    create: vi.fn(
      async ({
        ...
        data: Omit<TokenRow, 'id' | 'revokedAt'> & { revokedAt?: Date | null }
      }) => {
    ...
    updateMany: vi.fn(
      async ({ where, data }: { where: Partial<TokenRow>; data: Record<string, unknown> }) => {
        let count = 0
...
describe('auth.service', () => {
  ...
  describe('password reset', () => {
    it('revokes all outstanding refresh tokens when a reset completes', async () => {
      ...
      await expect(
        authService.login('frank@example.com', 'current-password'),
      ).rejects.toMatchObject({
        code: 'INVALID_CREDENTIALS',
      })
      await expect(
        authService.login('frank@example.com', 'brand-new-password'),
