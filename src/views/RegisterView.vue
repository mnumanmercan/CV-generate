<script setup lang="ts">
  ...
  const registerHeading = computed(() =>
    t_obj<{ prefix: string; accent: string; suffix: string }>('auth.register.heading'),
  )
  ...
  const passwordStrength = computed(() => passwordRules.value.filter((r) => r.met).length)
  ...
  <div class="min-h-screen flex flex-col" style="background: var(--paper)">
    ...
    <main class="flex-1 flex items-center justify-center px-4 py-14">
      <div class="w-full max-w-[440px] animate-slide-up">
        ...
        >
          {{ registerHeading.prefix }}<span class="accent-italic">{{ registerHeading.accent }}</span
          >{{ registerHeading.suffix }}
        </h1>
        ...
        <div class="paper-card p-7">
          ...
          <form class="space-y-5" novalidate @submit.prevent="handleSubmit">
            <!-- Full name -->
            <div>
              <label class="mono-eyebrow block mb-1.5" for="reg-name">{{
            ...
            <!-- Email -->
            <div>
              <label class="mono-eyebrow block mb-1.5" for="reg-email">{{
            ...
            <!-- Password -->
            <div>
              <label class="mono-eyebrow block mb-1.5" for="reg-password">{{
              ...
              <div class="relative">
                <input
                  ...
                  :class="[
                    'w-full px-4 py-2.5 pr-11 text-sm',
                    { 'input-error-flash': passwordFlash },
                  ]"
                />
                <button
                  type="button"
              ...
              <!-- Strength + requirements (visible once user starts typing) -->
              <Transition
                enter-active-class="transition-all duration-200 ease-out"
                enter-from-class="opacity-0 -translate-y-1"
                enter-to-class="opacity-100 translate-y-0"
              >
                <div v-if="password || passwordAttempted" class="mt-3">
                  <!-- Strength bar -->
                  <div class="flex items-center gap-1 mb-3">
                    <div
                      ...
                      :style="{
                        background: i <= passwordStrength ? strengthMeta.color : 'rgba(0,0,0,0.08)',
                      }"
                    />
                    <span
                      class="mono-eyebrow text-[10px] ml-2 w-9 text-right shrink-0 transition-colors duration-300"
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-2">
                    ...
                    >
                      <span
                        class="w-[18px] h-[18px] rounded-full flex items-center justify-center shrink-0 transition-all duration-200"
                        :style="
                          rule.met
                      ...
                      <span
                        class="mono-eyebrow text-[10.5px] transition-colors duration-200 leading-tight"
                        :style="{ color: rule.met ? '#15803D' : 'var(--muted)' }"
                        >{{ rule.label }}</span
                      >
                    </div>
                  </div>

                  <!-- Inline password error — appears only after a failed submit attempt -->
                  <Transition
                    enter-active-class="transition-all duration-200 ease-out"
                    enter-from-class="opacity-0 -translate-y-1"
                    enter-to-class="opacity-100 translate-y-0"
                  ...
                  >
                    <p
                      v-if="passwordAttempted && !passwordValid"
            ...
            <!-- Confirm password -->
            <div>
              <label class="mono-eyebrow block mb-1.5" for="reg-confirm">{{
              ...
              <div class="relative">
                <input
                  ...
                  :class="[
                    'w-full px-4 py-2.5 pr-11 text-sm',
                    { 'input-error-flash': confirmFlash },
                  ]"
                />
                <button
                  type="button"
            ...
            <div class="flex items-start gap-2.5">
              <input
                ...
                v-model="agreeTerms"
                type="checkbox"
                class="w-4 h-4 rounded mt-0.5 cursor-pointer shrink-0"
...
<style scoped>
  @keyframes error-flash {
    0% {
      box-shadow:
        0 0 0 0 rgba(180, 39, 39, 0),
        inset 0 0 0 0 rgba(180, 39, 39, 0);
    }
    18% {
      box-shadow:
        0 0 0 4px rgba(180, 39, 39, 0.16),
        inset 0 0 0 1.5px rgba(180, 39, 39, 0.5);
    }
    72% {
      box-shadow:
        0 0 0 4px rgba(180, 39, 39, 0.16),
  ...
  .input-error-flash {
    animation: error-flash 0.65s ease-in-out forwards;
  }
</style>
